...
![An enhanced "affamé" from Daryl Dixon (The Walking Dead Spinoff)](images/daryl-dixon-walker.avif){alt="An enhanced \"affamé\" from Daryl Dixon (The Walking Dead Spinoff)" fig-align="center"}
...
#Carly: I'm running into a problem where I am unable to run this chunk of code since you loaded as a csv file from your desktop. I would suggest having it read the file link directly from professor schmitt's github. This is how I did it if that's helpful: # zombies <- read.csv("https://raw.githubusercontent.com/fuzzyatelin/fuzzyatelin.github.io/refs/heads/master/AN588_Spring25/zombies.csv", header = TRUE, stringsAsFactors = FALSE) 
```
...
#Carly: creating matrices is such a good idea for visualizing the data clearly, i am definitely going to do this now too 
```
...
#Carly: This section was pretty much the same as I did it, I think it looks great! Love the colors you used.
```
...
#Carly: This also looks really similar to mine. If you wanted to make it look a little prettier to change the colors of the points in the scatterpoint you could use p7 <- p7 + scale_color_gradient(low = "blue", high = "green") or another set of colors. Just a suggestion for visual purposes and definitely not necessary but overall looks great.
```
...
#Carly: These are all again very similar to mine, I would suggest combining these into sections in a similar way you did for the question above where you plotted them in a grid. For example I used "par(mfrow = c(2, 3))" to combine all of my histograms into one page (just to make it easier to visualize).
```
...
samp_stats <- matrix(stat_array, nrow=10, ncol=1, byrow=TRUE)
rownames(samp_stats) <- names
colnames(samp_stats) <- "Value"
samp_stats

#Carly: Love the commentary here. To be concise you could also replace size = n in: "s <- sample_n(df, size = n, replace = FALSE)" so you could remove the n=30 in the line above it (since you don't use it again). 
#Carly: I also think it would be great in your array if you made two columns in the array to show one for mean and the other for sd: EX: 
stat_array <- c(s_h_mean, s_w_mean, s_a_mean, s_k_mean, s_e_mean, 
                s_h_sd, s_w_sd, s_a_sd, s_k_sd, s_e_sd)
samp_stats <- matrix(stat_array, ncol=2, byrow=FALSE)
rownames(samp_stats) <- c("Height", "Weight", "Age", "Zombies Killed", "Years of Education")
...
#Carly: Looks great, again love the matrix for displaying the CIs.
```
...
> They're all pretty close, the greatest difference is \~0.08 (weight)
...
#carly:  receiving a warning when running this section of code - something to be aware of "Warning: data length [505] is not a sub-multiple or multiple of the number of rows [100]"
```
...
**Overall Comments From Carly:** All comments above begin with #Carly (command f for easy finding of all comments made by me!)

-   I thought the way that Gentry sectioned out the multiple parts for each question was very smart and broke up the different parts of each question into a very manageable way. Also I appreciated how clear your comments were.

-   I also really liked how they created a matrix for the first question to present their results very clearly. I am definitely going to incorporate these two concepts into my final version of HW3.

-   I had very minor stylistic comments for Gentry regarding the presentation of the data that they generated to make it easier for others to visualize. These are again just suggestions and while I think that the way everything appears is great as is, these suggestions could be something to consider for their final version as potential changes to make.

-   When I attempt to render your website into an html, I don't see the `readthedown` theme or your toc applied. Maybe this would work better?
...
-   Ran into one warning in terms of data length in section 6.4, made a note of it above.
