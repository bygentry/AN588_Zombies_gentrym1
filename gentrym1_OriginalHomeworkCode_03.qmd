...
# create a sample, s1, of size 30 without replacement
n = 30
s1 <- sample_n(df, size = n, replace = FALSE)

# calculate the mean and standard deviation for each statistic of the sample
s1_h_mean <- mean(s1$height) # mean height of the sample
s1_h_sd <- sd(s1$height) # standard deviation of the height of the sample
s1_w_mean <- mean(s1$weight) # mean weight of the sample
s1_w_sd <- sd(s1$weight) # standard deviation of the weight of the sample
s1_a_mean <- mean(s1$age) # mean age of the sample
s1_a_sd <- sd(s1$age) # standard deviation of the age of the sample
s1_k_mean <- mean(s1$zombies_killed) # mean number of zombies killed of the sample
s1_k_sd <- sd(s1$zombies_killed) # standard deviation of the number of zombies killed of the sample
s1_e_mean <- mean(s1$education) # mean years of education of the sample
s1_e_sd <- sd(s1$education) # standard deviation of the years of education of the sample

# create an array of the sample stats for presentation
stat_array <- c(s1_h_mean, s1_h_sd, s1_w_mean, s1_w_sd, s1_a_mean, s1_a_sd, s1_k_mean, s1_k_sd, s1_e_mean, s1_e_sd)
...
# s1 refers to the sample created in 5.1
h_std_err <- s1_h_sd/sqrt(n)
w_std_err <- s1_w_sd/sqrt(n)
a_std_err <- s1_a_sd/sqrt(n)
k_std_err <- s1_k_sd/sqrt(n)
e_std_err <- s1_e_sd/sqrt(n)

# create lists of the standard errors, means, and variable names
sems <- c(h_std_err, w_std_err, a_std_err, k_std_err, e_std_err)
means <- c(s1_h_mean, s1_w_mean, s1_a_mean, s1_k_mean, s1_e_mean)
