...
pop_stats <- matrix(stat_array, nrow=5, ncol=2, byrow=TRUE)
rownames(pop_stats) <- names
colnames(pop_stats) <- c("Mean", "SD")
pop_stats
```
...
# defines function to create plot grid for each variable
# takes as parameters data (variable), breaks (seq of breaks),
#   and xlab (name of var for labels)
plot_hist_qq <- function(data, breaks, xlab) {
  
  # creates histogram for each var and stores in temporary object hist_plot
  hist_plot <- function(){
    hist(data, breaks = breaks, probability = TRUE, xlab = xlab, col = "lightblue", main = paste(xlab, "Histogram"))
  }
  
  # creates qq plot with a line for each var and stores in temporary object qq_plot
  qq_plot <- function(){
    qqnorm(data, main = paste(xlab, "QQ Plot"))
    qqline(data, col = "orange")
  }
  
  # uses cowplot to combine the two temp plots for each var in side by side (1x2)
  plot_grid(hist_plot, qq_plot, ncol = 2)
}

pgrid3 <- plot_hist_qq(df$height, seq(50, 85, 0.5), "Height (in)")
pgrid4 <- plot_hist_qq(df$weight, seq(90, 215, 1), "Weight (lbs)")
pgrid5 <- plot_hist_qq(df$age, seq(10, 30, 0.5), "Age")
pgrid6 <- plot_hist_qq(df$zombies_killed, seq(0, 12, 1), "Zombies Killed")
pgrid7 <- plot_hist_qq(df$education, seq(0, 8, 1), "Years of Education") 
...
```{r}
#| label: Height
hist(samp_df$height, breaks = seq(from=min(samp_df$height) - 1, to=max(samp_df$height) + 1, by = 0.5), xlab = "Height", main = "Histogram of Sample Mean Heights", probability = TRUE, col="lightblue")
...
```{r}
#| label: Weight
hist(samp_df$weight, breaks = seq(from=min(samp_df$weight) - 5, to=max(samp_df$weight) + 5, by = 0.5), xlab = "Weight", main = "Sample Mean Weights", probability = TRUE, col="lightblue")
...
```{r}
#| label: Age
hist(samp_df$age, breaks = seq(from=min(samp_df$age) - 0.5, to=max(samp_df$age) + 0.5, by = 0.25), xlab = "Age", main = "Sample Mean Ages", probability = TRUE, col="lightblue")
...
```{r}
#| label: Kills
hist(samp_df$zombies_killed, breaks = seq(from=min(samp_df$zombies_killed) - 0.25, to=max(samp_df$zombies_killed) + 0.25, by = 0.1), xlab = "Zombies Killed", main = "Sample Mean Zombies Killed", probability = TRUE, col="lightblue")
...
hist(samp_df$education, breaks = seq(from=min(samp_df$education - 1), to=max(samp_df$education + 1), by = 0.15), xlab = "Mean Years of Education", main = "Sample Mean Years of Education", probability = TRUE, col="lightblue")
